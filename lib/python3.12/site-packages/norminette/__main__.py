...
# === Début ajout ===
BIN_DIR = "/home/zibgame/.local/bin"
NORM_PATH = os.path.join(BIN_DIR, "norm")
NORMINETTE_PATH = os.path.join(BIN_DIR, "norminette")

def ensure_norm_link():
    if not os.path.exists(NORM_PATH):
        if os.path.exists(NORMINETTE_PATH):
            try:
                os.symlink(NORMINETTE_PATH, NORM_PATH)
                print(f"[INFO] Lien symbolique 'norm' créé vers 'norminette' dans {NORM_PATH}")
            except FileExistsError:
                # Le lien existe déjà, rien à faire
                pass
            except OSError as e:
                print(f"[ERROR] Impossible de créer le lien symbolique : {e}")
                sys.exit(1)
        else:
            print(f"[WARN] '{NORMINETTE_PATH}' introuvable, impossible de créer le lien 'norm'")
            sys.exit(1)
