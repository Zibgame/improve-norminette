...
def display_header(errors):
    ...
    # Récupère emoji et couleur depuis config avec fallback
    emoji = config.get("header_emoji", "🌊")
    color_name = config.get("header_color", "blue_light")

    # Dictionnaire des codes couleurs ANSI
    COLOR_CODES = {
        "blue_light": "\033[94m",
        "red": "\033[91m",
        "green": "\033[92m",
    ...
    # Récupère le code couleur, bleu clair par défaut
    color_code = COLOR_CODES.get(color_name.lower(), COLOR_CODES["blue_light"])
    BOLD = COLOR_CODES["bold"]
    RESET = COLOR_CODES["reset"]

    # Compose le header avec couleur + emoji
    header = f"{BOLD}{color_code}{emoji} ~~~ [ Norminette ] ~~~ {emoji}{RESET}\n"
...
def load_config():
    ...
    config_path = os.path.join(base_dir, "norminette_config.json")
