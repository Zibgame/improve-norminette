...
class HumanizedErrorsFormatter(_formatter):
    ...
    def _colorize_status(self, status: str) -> str:
        if not self.use_colors:
            return status
        if status == "OK":
            return f"\x1b[1m\x1b[32m{status}\x1b[0m"
        else:
            return f"\x1b[1m\x1b[31m{status}\x1b[0m"

    def __str__(self) -> str:
        output = ''
        for file in self.files:
            colored_status = self._colorize_status(file.errors.status)
            if file.errors.status == "Error":
                # nom fichier en gras + blanc pur
                filename_display = f"\033[1m\033[97m{file.basename}\033[0m"
            else:
                # nom fichier normal (pas de couleur/gras)
                filename_display = file.basename
            output += f"{filename_display}: {colored_status}!"
